...
def scale_quant_matrix(Q: np.ndarray, quality: int) -> np.ndarray:
    if quality < 1:
        quality = 1
    elif quality > 100:
        quality = 100
    ...
    return scaled_Q.astype(np.float32)

# --- DCT/IDCT Implementations ---
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", choices=["vector", "numba", "numba-parallel", "mp"], required=True)
    ...
    res = f"{arr.shape[0]}x{arr.shape[1]}"
    func = {
    ...
    Q_scaled = scale_quant_matrix(Q8, args.quality)

    t0 = time.perf_counter()
    for r in range(args.repeat):
        rec = func(arr, 8, 8, Q_scaled)
    dt = time.perf_counter() - t0
    print(f"{args.method},{res},{dt:.4f},quality={args.quality}")

    os.makedirs("output", exist_ok=True)
    out_fname = os.path.join("output", f"{args.method}_{res}_{args.repeat}_Q{args.quality}.png")
    Image.fromarray(rec).save(out_fname)
